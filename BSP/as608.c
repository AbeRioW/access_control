...
uint8_t UART2_RX_BUF[UART2_MAX_REC_LEN];


#define CPU_FRE_MHZ 72
...
{
	return HAL_UART_Transmit(&huart2,&data,1,0xffff);
...
{
	memset(UART2_RX_BUF,0,UART2_MAX_REC_LEN);  //发送前情况接收缓存,因为所有都要发送包头,所以值需要再发送包头前清空数据即可
...
static uint8_t *JudgeStr(uint16_t waittime)
{
	...
	while(--waittime)
	{
		...
		{
			USART2_RX_STA=0;
			data=strstr((const char*)UART2_RX_BUF,(const char*)str);
			if(data)
				return (uint8_t*)data;	
		}
	}
