...
{
		__HAL_UART_CLEAR_IDLEFLAG(huart); //清除中断

		RX_len = RXBUFFERSIZE - huart->RxXferCount; //计算接收数据长度
		HAL_UART_AbortReceive_IT(huart); //终止接收

		HAL_UART_Receive_IT(huart, (uint8_t*)aRxBuffer, RXBUFFERSIZE); //接收完数据后再次打开中断接收函数
}
